...
async function saveToDatabase(action, engine) {
    try {
        const { db } = await __turbopack_context__.A("[project]/src/db/index.ts [app-route] (ecmascript, async loader)");
        const { gameSessions, sessionScores, users } = await __turbopack_context__.A("[project]/src/db/schema.ts [app-route] (ecmascript, async loader)");
        ...
        if (action === "end" && state.sessionId) {
            ...
            for (const player of leaderboard){
                ...
                // Upsert user totals
                const existingUsers = await db.select().from(users).where(eq(users.tiktokUsername, player.tiktokUsername));
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0x4eq~w._.js.map